...
- Existing code passes `server_url=...` (Python) or `serverUrl: ...` (TS) with a non-default host.
- `HH_API_URL` is set in `.env`, deploy config, k8s manifest, or CI secrets.
...
- **401/403**: API key minted on a different deployment than `HH_API_URL`. Verify both came from the same workspace UI.
- **Spans silently dropping**: SDK defaulted to multi-tenant because `server_url`/`serverUrl` was not passed. Always pass explicitly.
- **TLS errors**: Self-host may use a custom CA — see [network-validation.md](network-validation.md) Step 3.
- **OTel collector silently exporting to default**: The OTLP exporter `endpoint:` doesn't auto-read `HH_API_URL`. Set it to `${env:HH_API_URL}/opentelemetry/v1/traces` explicitly. This failure is silent — only caught with verbose exporter logging.
